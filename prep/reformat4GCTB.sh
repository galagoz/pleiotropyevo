...
# Take SNP order into account though!!! Fucking genomic SEM doesn't output per SNP N....

awk 'NR==FNR{a[$1];next} $1 in a {print $1, $8}' GenomicSEM_multivarGWAS_dys_rhyimp_v2.tab ../MA/dys_aysnch_NGWAMA_results4GCTB.ma > perSNP_Ns_4genomicSEM.txt
sed -Ei '1s/^/SNP N\n/' perSNP_Ns_4genomicSEM.txt
# add perSNP Ns to gSEM_4gctb.ma
awk -F" " 'NR==FNR{a[NR]=$2;next} {$8=a[FNR]} 1' OFS=" " perSNP_Ns_4genomicSEM.txt GenomicSEM_multivarGWAS_dys_rhyimp_v2_4gctb.ma > GenomicSEM_multivarGWAS_dys_rhyimp_v2_4gctb_v2.ma
