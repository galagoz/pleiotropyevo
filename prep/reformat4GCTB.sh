...
awk '{print $1, $5, $6, $4, $12, $13, $15}' ${genSEM}GenomicSEM_multivarGWAS_dys_rhyimp_v2_secondrun.tab > ${genSEM}GenomicSEM_multivarGWAS_dys_rhyimp_v2_secondrun_4gctb.ma
...
awk 'NR==FNR{a[$1];next} $1 in a {print $1, $8}' ${genSEM}GenomicSEM_multivarGWAS_dys_rhyimp_v2_secondrun.tab ${genSEM}../MA/dys_aysnch_NGWAMA_results4GCTB.ma > ${genSEM}perSNP_Ns_4genomicSEM.txt
sed -Ei '1s/^/SNP N\n/' ${genSEM}perSNP_Ns_4genomicSEM.txt
# add perSNP Ns to gSEM_4gctb.ma
awk -F" " 'NR==FNR{a[NR]=$2;next} {print $0, a[FNR]}' ${genSEM}perSNP_Ns_4genomicSEM.txt ${genSEM}GenomicSEM_multivarGWAS_dys_rhyimp_v2_secondrun_4gctb.ma > tmp && mv tmp ${genSEM}GenomicSEM_multivarGWAS_dys_rhyimp_v2_secondrun_4gctb.ma
