...
newchisq = chisq/1.18 # dylexia paper says lambdaGC is 1.18, but LDSC outputs 1.572.
dys_ss$pvalue = pchisq(newchisq, df=1,lower.tail=FALSE)

fwrite(dys_ss, "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/data/dyslexia_1.18_GCcorr.dat", quote = F, sep = "\t", row.names = F, na="NA")
...
#
#fwrite(rhyimp_ss, "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/data/rhythm_impairment_GCcorr.dat", quote = F, sep = "\t", row.names = F, na="NA")
#
# 3) Genomic SEM CPM results

#gensem_ss = fread("/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_CPM_dys_rhyimp.tab")
#
#chisq = qchisq(gensem_ss$Pval_Estimate,1,lower.tail=FALSE)
#newchisq = chisq/1.623
#gensem_ss$Pval_Estimate = pchisq(newchisq, df=1,lower.tail=FALSE)
#
#fwrite(gensem_ss, "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_CPM_dys_rhyimp_GCcorr.tab", quote = F, sep = "\t", row.names = F, na="NA")
#
#N_hat_F1<-mean(1/((2*gensem_ss$MAF*(1-gensem_ss$MAF))*gensem_ss$SE^2), na.rm = T)
#gensem_ss$N = round(N_hat_F1)
#
#fwrite(gensem_ss[,c(1,4,5,6,14,15,22)], file = "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_CPM_dys_rhyimp_GCcorr_forMunge.tab", 
#       sep = "\t",  row.names = FALSE, col.names = TRUE)

# 4) Genomic SEM IPM results - dys

gensem_ss = fread("/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_dys_IPM.tab")

chisq = qchisq(gensem_ss$Pval_Estimate,1,lower.tail=FALSE)
newchisq = chisq/1.536
gensem_ss$Pval_Estimate = pchisq(newchisq, df=1,lower.tail=FALSE)

fwrite(gensem_ss, "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_dys_IPM_GCcorr.tab", quote = F, sep = "\t", row.names = F, na="NA")

N_hat_F1<-mean(1/((2*gensem_ss$MAF*(1-gensem_ss$MAF))*gensem_ss$SE^2), na.rm = T)
gensem_ss$N = round(N_hat_F1)

fwrite(gensem_ss[,c(1,4,5,6,14,15,22)], file = "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_dys_IPM_GCcorr_forMunge.tab",
      sep = "\t",  row.names = FALSE, col.names = TRUE)

# 5) Genomic SEM IPM results - rhyimp

gensem_ss = fread("/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_rhyimp_IPM.tab")

chisq = qchisq(gensem_ss$Pval_Estimate,1,lower.tail=FALSE)
newchisq = chisq/1.472
gensem_ss$Pval_Estimate = pchisq(newchisq, df=1,lower.tail=FALSE)

fwrite(gensem_ss, "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_rhyimp_IPM_GCcorr.tab", quote = F, sep = "\t", row.names = F, na="NA")

N_hat_F1<-mean(1/((2*gensem_ss$MAF*(1-gensem_ss$MAF))*gensem_ss$SE^2), na.rm = T)
gensem_ss$N = round(N_hat_F1)

fwrite(gensem_ss[,c(1,4,5,6,14,15,22)], file = "/data/clusterfs/lag/users/gokala/beat-dyslexiaevol/results/gSEM/GenomicSEM_multivarGWAS_rhyimp_IPM_GCcorr_forMunge.tab",
      sep = "\t",  row.names = FALSE, col.names = TRUE)
